...
from src.settings import BASE_PATH, ALL_CATEGORY, ALL_VECTOR
...
def reade_data(data_dir: str, category_list: list):
    """
    读取文件夹下所有文件特征量向量
    :param data_dir:
    :param category_list:
    ...
    for category in category_list:
        ...
        for parent, folders, files in os.walk(category_dir):
            logging.debug("parent:{0},folders:{1},files:{2}".format(parent, folders,
                                                            files))
...
def write_sample(category_list: list):
    """
    样本收集
    :param category_list:
    :return:
    ...
    with open(data_path, 'w') as f:
        ...
        f.writelines(reade_data(data_dir, category_list))
...
def regression_test():
    ...
    logging.debug('Loaded data file {0} with {1} rows'.format(filename, len(data_set)))

    train, test = split_data_set(data_set, 0.67)
    logging.debug('Split {0} rows into train with {1} and test with {2}'.format(
    ...
    for key, value in separated.items():
        ...
        logging.debug('Class: {0}  Attribute summaries: {1}'.format(key, summary))
